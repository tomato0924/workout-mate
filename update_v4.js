...
${calendarHeaderEndStr}`;
    ...
    // Chunk 2: From Calendar End to Modal Start
    // "            </Paper>" directly followed by something... Wait, what if there's multiple </Paper>? 
    // Yes, calendar view has </Paper>. Let's search backwards from "Competition Detail Section"
    const detailSectionStr = '            {/* Competition Detail Section */}';
    let detailSectionIdx = content.indexOf(detailSectionStr);
    
    // Reverse search for the first `            </Paper>` before `Competition Detail Section`
    let paperEndIdx = content.lastIndexOf('            </Paper>', detailSectionIdx);
    
    // We want the end of `            </Paper>` which is + 20 chars
