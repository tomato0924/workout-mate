...
// Normalize all newlines to linux style internally
...
${calendarHeaderEndStr}`;
    ...
    // Chunk 2 & 3: From Calendar End to Modal Start
    const endStr = '            </Paper>\\n\\n            {/* Competition Detail Section */}';
    const intersectionIndex = content.indexOf(endStr);
    
    // Search backwards for the Modal string starting from end of file
...
`;
    ...
    if (intersectionIndex === -1 || modalStartIndex === -1 || calendarHeaderStart === -1 || calendarHeaderEnd === -1) {
        console.log("calendarHeaderStart:", calendarHeaderStart);
        console.log("calendarHeaderEnd:", calendarHeaderEnd);
        console.log("intersectionIndex:", intersectionIndex);
        console.log("modalStartIndex:", modalStartIndex);
        throw new Error("Missing indexes");
    }

    content = content.substring(0, intersectionIndex) + chunk2 + content.substring(modalStartIndex);

    // Convert back to CRLF before saving if it was CRLF originally
