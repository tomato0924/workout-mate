...
// Chunk 2 & 3: Replace from Calendar End to Modal Start
// Find `            </Paper>` right before `            {/* Competition Detail Section */}`
const match = content.match(/            <\/Paper>\\r?\\n\\s*\{\/\* Competition Detail Section \*\/\}/);
if (!match) throw new Error("Could not find intersection of Calendar End and Detail Section");
const intersectionIndex = match.index;

const modalStartMatch = content.match(/\\s*\{\/\* Create \/ Edit Modal \*\/\}/);
// We want the modalStartMatch that comes AFTER intersectionIndex
let modalStartIndex = -1;
for (const m of content.matchAll(/\\s*\{\/\* Create \/ Edit Modal \*\/\}/g)) {
    if (m.index > intersectionIndex) {
        modalStartIndex = m.index;
        break;
    }
}
if (modalStartIndex === -1) throw new Error("Could not find Modal start");
...
content = content.substring(0, intersectionIndex) + chunk2 + content.substring(modalStartIndex);
